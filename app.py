...
with st.form("prediction_form"):
    ...
    Scholarship_holder = st.selectbox("Apakah mendapatkan mahasiswa?", list(Scholarship_holder_options.keys()))
    ...
    if submitted:
        ...
        # Tampilkan hasil
        if prediction[0] == 1:
            st.error(f"⚠️ Mahasiswa diprediksi akan *Dropout*. Probabilitas: {round(probability[0][1]*100, 2)}%")
        else:
